...
from anketa_recommendations import anketa_reco_start, users_movies, final_reco, anketa_dontknow_reco
from anketa_random import anketa_random_start, anketa_random_result, final_random, anketa_dontknow_random
...
def main():
    ...
    states={
        ...
        'final_reco': [MessageHandler(Filters.regex('^(Я нашел нужный фильм)$'), final_reco)]
    ...
    states={
        'anketa_random_result': [MessageHandler(Filters.regex('^(Получить фильм|Подобрать другой случайный фильм)$'), anketa_random_result)],
        'final_random': [MessageHandler(Filters.regex('^(Подобрать другой случайный фильм|Я нашел нужный фильм)$'), final_random)]
